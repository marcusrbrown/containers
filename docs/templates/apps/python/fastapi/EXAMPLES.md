...
poetry run template-engine generate apps/python/fastapi ./my-Python FastAPI-dev \
  --param environment=development \
  --param debug=true

cd my-Python FastAPI-dev
...
poetry run template-engine generate apps/python/fastapi ./my-Python FastAPI-prod \
  --param environment=production \
  --param enable_ssl=true \
  --param security_hardening=true

cd my-Python FastAPI-prod

# Build production image
docker build -t my-Python FastAPI:prod .

# Run with production settings
docker run -d \
  --name my-Python FastAPI-prod \
...
```bash
# Generate app
poetry run template-engine generate apps/python/fastapi ./my-app
...
for i in {1..3}; do
  poetry run template-engine generate apps/python/fastapi ./app-$i \
  ...
  build:
    ...
    steps:
      ...
      - name: Generate from template
        run: |
          poetry install
          poetry run template-engine generate apps/python/fastapi ./app
...
poetry run template-engine test apps/python/fastapi

# Test with custom parameters
poetry run template-engine test apps/python/fastapi --params test-params.json

# Run comprehensive tests
poetry run template-testing apps/python/fastapi --integration --performance
```
...
```bash
poetry run template-engine generate apps/python/fastapi ./custom-app \
...
```bash
# Development
poetry run template-engine generate apps/python/fastapi ./dev \
  --param environment=dev \
  --param debug=true \
  --param hot_reload=true

# Staging
poetry run template-engine generate apps/python/fastapi ./staging \
  --param environment=staging \
  --param monitoring=true \
  --param ssl=true

# Production
poetry run template-engine generate apps/python/fastapi ./prod \
